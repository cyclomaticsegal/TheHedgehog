...
pub enum FoldsResult {
    ...
    /// Refresh reached the server and the server confirmed the build
    /// is in the "Failed" state. Distinct from `RefreshFailed` (which
    /// is a network/parse error) — this one means "the server knows
    /// this model failed, offer a Retry".
    RefreshFoundFailed { model_id: String },
}
...
pub fn refresh_model(
    ...
    rt.block_on(async {
        ...
        match client.models().get(&model_id, None, None).await {
            Ok(model) => {
                // Server-confirmed failure takes precedence over the
                // emptiness heuristic: a Failed model is expected to
                // have empty outcomes/drivers, and telling the user
                // "try again in a moment" would be wrong — it won't
                // ever succeed without a retry.
                if model.is_failed() {
                    storage::update_folds_model_status_standalone(
                        &db_path,
                        &model_id,
...
// ---------------------------------------------------------------------------
// Retry a failed model build
// ---------------------------------------------------------------------------

/// POST `/api/v1/models/{id}/retry` and poll the resulting build to
/// completion. The server re-enters the same async lifecycle as a
/// fresh `create`, so we flip the local DB row back to "pending" and
/// reuse the same polling/terminal-state handling as `create_and_poll`.
///
/// The response is delivered over the same channel the UI uses for a
/// normal foreground build — `Created` once the retry is accepted,
/// then `Completed`/`Failed` when polling terminates.
pub fn retry_build(
    api_key: String,
    model_id: String,
    db_path: PathBuf,
    ...
    rt.block_on(async {
        ...
        tracing::info!(model_id = %model_id, "folds: POST retry");
        if let Err(e) = client.models().retry(&model_id).await {
            tracing::warn!(
                model_id = %model_id,
                error = %e,
                "folds: retry POST failed"
            );
            let _ = tx.send(FoldsResult::Failed(format!("{e}")));
            return;
        }
        tracing::info!(model_id = %model_id, "folds: retry accepted");

        // Flip the DB row back to "pending" so the resume-on-restart
        // sweep would pick this build up if the app closes mid-retry.
        storage::update_folds_model_status_standalone(
            &db_path,
            &model_id,
        ...
        {
            Ok(model) => {
                tracing::info!(model_id = %model_id, "folds: retry complete");
                persist_completed(&db_path, &model_id, &model);
                let _ = tx.send(FoldsResult::Completed(Box::new(model)));
            }
            Err(FoldsError::ModelBuildFailed { model_id: mid, .. }) => {
                tracing::warn!(model_id = %mid, "folds: retry failed server-side");
                storage::update_folds_model_status_standalone(
                    &db_path,
                    &mid,
                    FOLDS_STATUS_FAIL,
                ...
                let _ = tx.send(FoldsResult::Failed(format!("Model {mid} retry failed")));
            }
            Err(FoldsError::PollTimeout { message }) => {
                tracing::warn!(
                    model_id = %model_id,
                    message = %message,
                    "folds: retry poll timeout"
                );
                storage::update_folds_model_status_standalone(
                    &db_path,
                    &model_id,
            ...
            Err(e) => {
                tracing::warn!(
                    model_id = %model_id,
                    error = %e,
                    "folds: retry unexpected poll error"
                );
                let _ = tx.send(FoldsResult::Failed(format!("{e}")));
            }
        }
