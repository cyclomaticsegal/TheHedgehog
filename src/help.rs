...
Keys are loaded from `.env` on startup. You can also enter or update them in the sidebar and click **Save**, which writes them back to your `.env` file. Keys are never stored in the database.
...
- **Analyze Current View**: sends the current VIX status, primary/secondary/tertiary instrument tiers, 30-day price changes, and detected spike episodes to the LLM along with the built-in knowledge base. The response appears in the AI Analysis panel at the bottom.

Browsing past analyses lives in the **Summary Report** view: date-range filter, selectable list, click any row to reload its full response.
...
The log appears automatically when a refresh starts. Dismiss it with the × button to hide the panel; the log content is preserved. To reopen it, click the **Activity** button that appears in the status bar. A new refresh also reopens the panel automatically.
...
- The **primary instrument** you've chosen as the hypothesis subject
- Any **secondary instruments** selected alongside (corroborative signal only, not the subject)
- **Tertiary instruments** (not selected) as background context
...
2. Propose a **hypothesis**: a specific, time-bounded, mechanism-named claim about where the primary instrument is going over the next 7–90 days
...
Every analysis has exactly one **primary** instrument: the grammatical subject of the hypothesis and the source of every strike level and outcome-band price. Other selected instruments are **secondary**: named in the hypothesis context as corroborative or challenging signal, but never as subject. Unselected instruments are **tertiary**: background only, mentioned only when they materially support or contradict the primary thesis.

When you click **Analyze Current View**:
- With **one** instrument selected, the primary is trivially that one, so analysis runs immediately.
- With **more than one** selected, a dialog appears asking *"Pick the primary instrument for this analysis"* with a radio button per selected instrument (first one pre-ticked). Pick one, click Analyze, and the remaining selected instruments become secondaries for that run.

This framework lets the bias checks and LLM judge discriminate cleanly. See ADR 0022 for the rationale.

### Bias-failure feedback on Re-analyze

The LLM bias judge (see *Commodity-Bias Validation* below) runs after every analysis and may flag issues, e.g. *MECHANISM_RELEVANCE: generic macro framing rather than primary-specific transmission channels*.

If you click **Re-analyze** and the primary hasn't changed, the app automatically injects the flagged failures into the system prompt as a corrective *"PRIOR ATTEMPT: BIAS FAILURES TO ADDRESS"* block. The next run sees the judge's specific complaints and can course-correct. The stash clears on dispatch, so only the most recent run's failures feed forward; they don't accumulate.

If you change the primary before re-analyzing, the stash is skipped (the old failure reasons would misalign with the new subject).
...
Every analysis response is automatically saved to the local database with a timestamp, the VIX reading at the time, and the full prompt context. You never need to manually save an analysis. Past analyses are browsable from the **Summary Report** view: date-range filter, selectable list, click a row to reload the full response.

### Panel visibility
Close the AI panel with the × button. To reopen it, click the **AI** button in the status bar, click a row in the Summary Report inference list, or run a new analysis.
...
- The user message names exactly one primary instrument and the response's hypothesis treats that instrument as its subject
- The primary appears in the "Latest close" block; secondaries and tertiaries do not
- When secondaries exist, the "Secondary instruments" block is present with the corroborative-only framing; when empty, the block is absent
- No selected instrument (primary or secondary) leaks into the tertiary block
- Dollar amounts in the hypothesis are anchored to the primary's latest close, not stale training-data figures (e.g. citing gold at $2,000 when the latest close is $4,624)

**Semantic checks (async, LLM judge)**: a second, focused LLM call that validates the analysis against five rules:
- **SUBJECT_MATCH**: is the hypothesis subject exactly the PRIMARY? Not a secondary, not a tertiary.
- **PRICE_ANCHORING**: are the strike levels and outcome-band prices consistent with the PRIMARY's latest close?
- **MECHANISM_RELEVANCE**: is the causal mechanism specific to the PRIMARY, or generic macro filler?
- **SECONDARY_FRAMING**: are secondaries named as corroborative/challenging signal (never as subject), and are tertiaries kept as background only?
- **OUTCOME_ALIGNMENT**: do outcome bands reference the PRIMARY's price level with distinct causal paths?
...
In both cases, the panel shows "Model ID: X   building…" while the model is under construction (typically 45–75 minutes for the Advanced tier; the app marks anything still pending past 2 hours as `undisclosed_failure`). When the model completes, the sidebar summary switches to green **Model ID: X   complete** with the outcome probabilities listed below, and the central panel auto-switches from VIX charts to **51Folds**.

Models persist across restarts. If you close the app while a model is building, the resume sweep will re-attach the polling thread on next launch for any rows still within the 2-hour window and update the sidebar once the SDK reports completion.

### Recovering from a failed build

If you reload a completed inference from the **Loaded Inferences** list and its 51Folds build failed server-side, the Report view shows **Build failed on 51Folds** with a **Retry build** button instead of the generic "reload from 51Folds" path. Retry reruns the same model ID; no need to rebuild the hypothesis from scratch, and the downstream inference row stays linked. The retry flows through the normal foreground slot, so the spinner, tray chip, elapsed timer, and sidebar badge all behave as if it were a fresh build.

A genuinely-transient refresh failure (network error, server not yet done) still surfaces the original "Try again in a moment" banner. The Retry path appears only when the server has confirmed the build is in the Failed state.

### Concurrent builds

You can run multiple 51Folds builds in parallel. Each Create click starts its own build; any build already in flight is parked into a backlog (keeping its polling thread alive), and the newest one becomes the foreground view.

A toolbar **tray chip** (`◌ N building`) appears in the top toolbar to the right of the 51Folds tab whenever any build is active. Click it to see a popover listing each active build with its model ID and elapsed time. Click a backlog entry to swap it into the foreground; the current foreground is parked, and the chosen build becomes visible.

When you click a row in the Summary Report inference list whose linked model is still building in this session, the app swaps that build into the foreground from the backlog rather than trying to fetch a (stub) copy from the server.

### Central panel views

The central panel has several top-level views selectable from the toolbar:

- **VIX charts**: VIX time series, asset-performance-vs-VIX comparison chart, optional price panel (press `[P]`). This is the default view.
...
- **Help**: this content, rendered inline in the central panel. Click **Help** in the top bar to toggle it.
...
3. Browse the loaded list. Each entry shows its timestamp, source (Analysis or Report), VIX reading, 51Folds model badge, and hypothesis preview. Hover for full text; click to load the inference into the sidebar. Badges (□ pending, ● success, ⚠ failed) refresh live as builds progress, and a failed badge promotes back to pending → success if you retry and the rerun succeeds.
