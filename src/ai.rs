...
fn call_llm(request: &AiRequest) -> Result<String> {
    // Disable redirect following: the API key lives in a header and reqwest's
    // default policy would forward it to any redirect target (e.g. a rogue
    // CDN response) before we could inspect the hop.
