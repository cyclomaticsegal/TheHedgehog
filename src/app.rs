...
use dialogs::{
    api_key_field, api_key_field_with_hint, back_button, collapsible_chart_header,
    diff_model_states, empty_state_panel, hedgehog_terminal_theme, render_analyze_primary_dialog,
    render_apply_confirm_dialog, render_reeval_error_banner, render_reeval_in_flight_banner,
    render_reeval_success_toast, section_card, status_banner,
};
use inference_label::inference_label;
use sidebar::{
    sidebar_overlay_controls, sidebar_spike_episodes, sidebar_threshold_controls,
    sidebar_vix_summary,
};
use tasks::{
...
/// `&mut self` method) for historical reasons; now called from the
/// central-panel Help view.
...
impl DashboardApp {
    ...
    fn spawn_research_terminal(&mut self) {
        ...
        // present in the environment. Every interpolated value is passed
        // through `sh_single_quote` so stray apostrophes in a key, env var,
        // or $HOME-derived path can't break out of the quoting.
        let mut exports = format!("export {env_key_name}={}", sh_single_quote(&api_key));
        ...
        for key in passthrough_keys {
            if let Ok(val) = std::env::var(key)
                && !val.is_empty()
            {
                exports.push_str(&format!(" && export {key}={}", sh_single_quote(&val)));
            }
        }
        ...
        exports.push_str(&format!(
            " && export HEDGEHOG_DB_PATH={}",
            sh_single_quote(&db_path_str)
        ));
...
latest close. Keep SECONDARY instruments as corroborative mentions only.\n",
    ...
    fn poll_ai(&mut self) {
        match self.ai_task.poll() {
            LlmPoll::Response(result) => {
                ...
                // Park any in-flight / visible 51Folds build before
                // resetting the foreground — otherwise a fresh analysis
                // silently drops the channel and the build disappears
                // from the tray while still running on the server.
                // Same invariant ADR 0020 established for
                // load_historical_inference and start_folds_create.
    ...
    /// Retry a server-confirmed failed build. Consumes
    /// `refresh_found_failed_id` and spawns a `retry_build` thread that
    /// POSTs `/retry`, updates the DB to pending, and polls until
    /// terminal. Reuses the foreground `FoldsTask` (parking any
    /// existing build to the backlog first) so the tray spinner,
    /// elapsed-time readout, and completion toast all flow through the
    /// normal build lifecycle.
    fn start_folds_retry(&mut self) {
        let api_key = self.api_keys.folds.trim().to_owned();
        if api_key.is_empty() {
            self.folds_task.error = Some("No 51Folds API key set.".to_owned());
            return;
        }
        let Some(model_id) = self.folds_task.refresh_found_failed_id.take() else {
            return;
        };
        // A failed inference loaded from the Loaded Inferences list
        // populates `draft_hypothesis` but not `folds_task.question`,
        // so fall back to the draft (and to `last_inference_id`) so
        // the tray chip reads the real question rather than
        // "(untitled)".
        let question_label = self
            .folds_task
            .question
            .clone()
        ...
        // Park any in-flight or still-visible foreground build before
        // re-purposing the task slot, same invariant as
        // `start_folds_create`.
        if self.folds_task.in_flight || self.folds_task.model.is_some() {
            let parked = std::mem::replace(&mut self.folds_task, FoldsTask::new());
            self.folds_backlog.park(parked);
        ...
        let (tx, rx) = mpsc::channel();
        self.folds_task.start(rx);
        self.folds_task.model_id = Some(model_id.clone());
        self.folds_task.question = question_label;
        self.folds_task.started_at = Some(chrono::Utc::now());
        self.folds_task.inference_id = inference_id;

        let db_path = database_path();
        thread::spawn(move || {
            crate::folds::retry_build(api_key, model_id, db_path, tx);
        });
    }
    ...
    fn poll_folds(&mut self) {
        ...
        let was_refreshing = self.folds_task.refresh_in_flight;
        ...
        // A successful Refresh (user clicked a historical inference row
        // that had been marked failed/undisclosed, and the SDK returned
        // a complete model) means `persist_completed` already wrote
        // status="success" to the DB. Pull that back into the in-memory
        // badge map so the red "!" badge flips to green without needing
        // a restart.
        let refresh_succeeded = was_refreshing
            && !self.folds_task.refresh_in_flight
            && self.folds_task.refresh_error.is_none();
        if !backlog_touched.is_empty() || foreground_terminal || refresh_succeeded {
            self.reload_folds_status_map();
        }
    ...
    fn render_folds_tray(&mut self, ui: &mut egui::Ui) {
        ...
        let chip = ui
            .horizontal(|ui| {
                ui.add(egui::Spinner::new().size(12.0).color(ACCENT_BLUE));
                ui.small_button(RichText::new(chip_text).color(ACCENT_BLUE))
            })
            .inner
    ...
    fn render_folds_hypothesis_editor(&mut self, ui: &mut egui::Ui) {
        ...
        ui.label(
            RichText::new("Hypothesis Statement")
                .size(12.0)
                .strong()
                .color(TEXT_PRIMARY),
        );
        ui.label(
            RichText::new(
                "Substantive claim (not a question), time-bounded 7-90 days, explaining mechanism.",
            )
            .size(11.0)
            .color(TEXT_SECONDARY),
        );
...
51Folds to generate an Advanced-tier Bayesian model.",
    ...
    fn render_folds_model_results(&mut self, ui: &mut egui::Ui) {
        ...
        for (label, prob) in &outcomes {
            ...
            if let Some(ref prev) = previous
                && let Some((_, prev_prob)) = prev.iter().find(|(l, _)| l == label)
            {
    ...
    /// Render the help content as a central panel view (formerly a
    /// floating dialog window). Centered 720px column to match the
    /// Report view's reading width, with the mascot header + full
    /// markdown body in a single scroll area.
    fn render_central_help_view(&mut self, ui: &mut egui::Ui) {
        self.ensure_mascot_texture(ui.ctx());
        let max_width = 900.0_f32;
        let available_w = ui.available_width();
        let side_pad = ((available_w - max_width) / 2.0).max(0.0);

        egui::ScrollArea::vertical()
            .id_salt("help_central_scroll")
            .auto_shrink([false, false])
            .show(ui, |ui| {
                ui.add_space(20.0);
                ui.horizontal(|ui| {
                    ...
                    ui.vertical(|ui| {
                        ...
                        render_help_header(ui, self.mascot_texture.as_ref());
                        egui_commonmark::CommonMarkViewer::new()
    ...
    fn render_back_pill(&mut self, ui: &mut egui::Ui) {
        ...
        let label_text = match target {
            ...
            CentralView::Help => "\u{2190} Back to Help",
        };
        ui.add_space(6.0);
        ui.horizontal(|ui| {
            ui.add_space(12.0);
        ...
        if !self.folds_task.is_complete() {
            ...
            ui.vertical_centered(|ui| {
                if let Some(id) = recoverable_id {
                    if self.folds_task.refresh_in_flight {
                    ...
                    } else if self.folds_task.refresh_found_failed_id.is_some() {
                        // Server-confirmed failure. Offer a Retry
                        // rather than another Reload (Reload would
                        // just re-read the same failed state).
                        ui.label(
                            RichText::new("Build failed on 51Folds")
                                .size(18.0)
                                .color(ALERT_EXTREME_FG),
                        );
                        ui.add_space(8.0);
                        ui.add(
                            egui::Label::new(
                                RichText::new(format!(
                                    "The server reports model {id} failed to build. Retry to queue a fresh build under the same ID.",
                                ))
                                .size(13.0)
                                .color(TEXT_SECONDARY),
                            )
                        ...
                        if ui
                            .add(
                                egui::Button::new(
                                    RichText::new("Retry build on 51Folds")
                                        .size(13.0)
                                        .strong()
                                        .color(Color32::WHITE),
                                )
...
impl DashboardApp {
    ...
    fn render_splash(&mut self, ctx: &egui::Context) {
        ...
        if user_skipped
            && elapsed_ms < HOLD_MS
            && let Some(start) = self.splash.shown_at
        {
        ...
        egui::TopBottomPanel::top("top_bar").show(ctx, |ui| {
            ui.horizontal_wrapped(|ui| {
                ...
                if ui.button("Help").clicked() {
                    // Toggle: if already on Help, return to the
                    // default Charts view; otherwise navigate to Help.
                    if self.central_view == CentralView::Help {
                        self.central_view = CentralView::Charts;
                    } else {
                        self.central_view = CentralView::Help;
                    }
                    self.view_back_target = None;
                }
            });
    ...
    fn show_dashboard(&mut self, ctx: &egui::Context) {
            ...
            ui.horizontal_wrapped(|ui| {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    if !self.ai_panel_open
                        ...
                        && ui.small_button("AI").on_hover_text("Show AI Analysis panel").clicked()
                    {
                        self.ai_panel_open = true;
                    }
                    if !self.show_activity_log
                        && !self.activity_log.is_empty()
                        && ui.small_button("Activity").on_hover_text("Show Activity log").clicked()
                    {
                        self.show_activity_log = true;
                    }
                });
        ...
        egui::CentralPanel::default().frame(central_frame).show(ctx, |ui| {
            match self.central_view {
                ...
                CentralView::Help => {
                    self.render_central_help_view(ui);
                }
            }
        });
...
fn paint_chart(
    ...
    if is_dragging
        && let (Some(start_x), Some(pos)) = (*drag_start, response.interact_pointer_pos())
    {
        let ratio =
    ...
    if response.drag_stopped_by(egui::PointerButton::Primary)
        && let (Some(start_x), Some(pos)) = (drag_start.take(), response.interact_pointer_pos())
    {
        let ratio =
            ((pos.x - chart.left()) / (chart.right() - chart.left())).clamp(0.0, 1.0);
